...
| `ViaCastingUI` | `TimelinePollCard`, `SponsorBadge`, `ContestCard` | Sports casting UI |
...
struct MyApp: App {
    init() {
        // 1. Load vio-config.json → applies theme, sets up campaign URLs and API key
        ConfigurationLoader.loadConfiguration(userCountryCode: "NO")
        // 2. CampaignManager initializes (autoDiscover mode — waits for setBroadcastContext)
    }
}

// Later, when user opens a stream:
// 3. setBroadcastContext → discoverCampaigns → WebSocket connects → polls/contests arrive
await CampaignManager.shared.setBroadcastContext(context)
```
...
- [Configuration](/swift/configuration) — `vio-config.json` options
